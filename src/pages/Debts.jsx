...
import { Plus, X, Landmark, AlertCircle, Trash2, CheckCircle, TrendingDown, Calendar, CreditCard, PieChart } from 'lucide-react'
...
export default function Debts() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setShowForm(false)
      fetchDebts()
      setForm({ name: '', total_amount: '', interest_rate: '0', total_installments: '1', start_date: new Date().toISOString().slice(0, 10) })
    } catch (err) {
  ...
  const deleteDebt = async (id) => {
    if (!confirm('¿Seguro que deseas eliminar esta deuda permanentemente?')) return
    try {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {loading ? (
          ...
          debts.map(debt => {
            ...
            return (
              <div key={debt.id} className="glass-panel p-6 rounded-3xl relative overflow-hidden group">
                {isPaid && (
                ...
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h3 className="text-xl font-black text-deep-900 dark:text-white tracking-tight">{debt.name}</h3>
                  ...
                  {!isPaid && (
                    <button onClick={() => deleteDebt(debt.id)} className="p-2 text-gray-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-colors dark:text-gray-600 dark:hover:bg-white/5">
                ...
                {!isPaid && (
                  <div className="border-t border-gray-100 dark:border-white/5 pt-5 mt-2 flex justify-end">
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-deep-950/80 backdrop-blur-md animate-in fade-in duration-300">
           <div className="w-full max-w-lg bg-white dark:bg-deep-900 rounded-[2.5rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden border border-white/10 animate-in zoom-in-95 duration-300">
             ...
             <div className="px-8 py-6 border-b border-gray-100 dark:border-white/5 flex justify-between items-center bg-gray-50/50 dark:bg-black/20">
               <h2 className="text-2xl font-black text-deep-900 dark:text-white tracking-tight">Nueva Deuda</h2>
             ...
             <div className="p-8 overflow-y-auto custom-scrollbar">
               ...
               <form id="debt-form" onSubmit={handleSave} className="space-y-6">
                 <div>
                   <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Nombre o Entidad</label>
                 ...
                 <div className="grid grid-cols-2 gap-5">
                   <div className="col-span-2 sm:col-span-1">
                     <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Monto Préstamo</label>
                 ...
                 <div className="grid grid-cols-2 gap-5">
                   <div className="col-span-2 sm:col-span-1">
                     <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Número de Cuotas</label>
                     <input type="number" required value={form.total_installments} onChange={e => setForm({...form, total_installments: e.target.value})}
                       className="w-full rounded-2xl border border-gray-200 px-5 py-3.5 text-sm font-bold focus:border-mint-500 focus:ring-mint-500 shadow-inner dark:bg-deep-950 dark:border-white/10 dark:text-white transition outline-none" />
                   </div>
                   <div className="col-span-2 sm:col-span-1">
                     <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Fecha Desembolso</label>
                     <input type="date" required value={form.start_date} onChange={e => setForm({...form, start_date: e.target.value})}
                       className="w-full rounded-2xl border border-gray-200 px-5 py-3.5 text-sm font-bold focus:border-mint-500 focus:ring-mint-500 shadow-inner dark:bg-deep-950 dark:border-white/10 dark:text-white text-gray-500 transition outline-none" />
                   </div>
                 </div>

                 <div className="bg-mint-50 dark:bg-deep-950 p-5 rounded-2xl border border-mint-100 dark:border-white/5 mt-6">
                   <p className="text-xs font-bold text-mint-600 dark:text-mint-400 uppercase tracking-widest mb-1">Predicción de Cuota Mensual Fija</p>
                   <p className="text-3xl font-black text-deep-900 dark:text-white tracking-tight">{formatCOP(generatedInstallment)}</p>
                 </div>
               </form>
             </div>

             <div className="p-6 border-t border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-deep-900/50 flex justify-end gap-3 mt-auto">
               <button onClick={() => setShowForm(false)} className="px-6 py-3 text-sm font-bold text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 dark:bg-deep-800 dark:text-gray-300 dark:border-white/10">Cancelar</button>
               <button type="submit" form="debt-form" disabled={submitting} className="px-8 py-3 text-sm font-bold text-white bg-gradient-to-r from-mint-500 to-mint-600 hover:from-mint-600 hover:to-mint-700 rounded-xl shadow-lg shadow-mint-500/30">Guardar Deuda</button>
             </div>
           </div>
        </div>, document.body
      )}
